...
import { Check, Mail, MessageCircle, Plus, Trash2, Pencil, Sparkles, Copy, CheckCircle2, LogOut, CalendarPlus, Tag, Image as ImageIcon, Link2, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Home, Instagram, FileDown } from 'lucide-react';
...
.mbt-header-actions { display: flex; align-items: center; gap: 8px; }
.mbt-backhome {
  display: flex;
  align-items: center;
  gap: 6px;
...
.mbt-backhome:hover { border-color: var(--rose); color: var(--rose); }
...
/* Admin portfolio manager (Website tab) */
.mbt-portfolio-grid {
  display: grid;
...
.mbt-portfolio-item img { width: 100%; height: 100%; object-fit: cover; display: block; }
.mbt-portfolio-draggable { cursor: grab; transition: opacity 0.15s ease, transform 0.15s ease; }
.mbt-portfolio-draggable:active { cursor: grabbing; }
.mbt-portfolio-dragging { opacity: 0.4; transform: scale(0.96); }
...
/* Public homepage — Rhode-inspired editorial redesign */
.mbt-rhode {
  --stone: #FBF7F1;
  --paper: #FFFFFF;
  --ink: #1E1A17;
...
.mbt-rhode * { box-sizing: border-box; }
...
.mbt-rhode-navlink:hover { border-bottom-color: var(--ink); }
.mbt-rhode-navcta {
  font-family: 'Inter', sans-serif;
  font-size: 12px;
  letter-spacing: 0.08em;
...
.mbt-rhode-navcta:hover { background: var(--ink); color: var(--paper); }
.mbt-rhode-social { display: flex; align-items: center; gap: 14px; }
.mbt-rhode-social a { color: var(--ink); display: flex; opacity: 0.75; transition: opacity 0.15s ease; }
.mbt-rhode-social a:hover { opacity: 1; color: var(--rose); }
...
.mbt-rhode-hero-empty { background: linear-gradient(160deg, var(--stone), var(--paper)); min-height: 60vh; }
.mbt-rhode-hero-img {
  position: absolute;
  inset: 0;
  width: 100%;
...
.mbt-rhode-hero-scrim {
  position: absolute;
  inset: 0;
  z-index: 0;
  background: linear-gradient(180deg, rgba(10,8,7,0.08) 0%, rgba(10,8,7,0.32) 40%, rgba(10,8,7,0.78) 100%);
}
.mbt-rhode-hero-copy {
  position: relative;
  z-index: 1;
...
.mbt-rhode-hero-empty .mbt-rhode-hero-copy { color: var(--ink); }
.mbt-rhode-eyebrow {
  font-family: 'Inter', sans-serif;
  font-size: 11px;
  letter-spacing: 0.18em;
...
.mbt-rhode-hero-title {
  ...
  margin: 0 0 26px;
  max-width: 700px;
  text-shadow: 0 2px 6px rgba(0,0,0,0.35), 0 8px 32px rgba(0,0,0,0.3);
}
.mbt-rhode-hero-empty .mbt-rhode-hero-title { text-shadow: none; }
.mbt-rhode-hero-cta {
  display: inline-block;
  font-family: 'Inter', sans-serif;
  font-size: 13px;
...
.mbt-rhode-hero-cta:hover { background: currentColor; }
.mbt-rhode-hero-cta:hover { color: var(--ink); }
.mbt-rhode-hero-empty .mbt-rhode-hero-cta:hover { color: var(--paper); }
...
.mbt-rhode-about-photo {
  aspect-ratio: 4/5;
  overflow: hidden;
  border-radius: 3px;
}
.mbt-rhode-about-photo img { width: 100%; height: 100%; object-fit: cover; }
.mbt-rhode-about-copy p {
  font-family: 'Inter', sans-serif;
  font-size: 15px;
  line-height: 1.75;
  color: var(--ink-soft);
...
.mbt-rhode-portfolio { max-width: 1200px; margin: 0 auto; padding: 30px 32px 100px; }
.mbt-rhode-portfolio-grid {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 3px;
}
.mbt-rhode-portfolio-item { display: block; aspect-ratio: 4/5; overflow: hidden; }
.mbt-rhode-portfolio-item img {
  width: 100%;
  height: 100%;
  object-fit: cover;
  display: block;
...
.mbt-rhode-portfolio-item:hover img { transform: scale(1.05); }

.mbt-rhode-pricing { max-width: 720px; margin: 0 auto; padding: 30px 32px 100px; }
.mbt-rhode-price-list {
  display: flex;
...
.mbt-rhode-price-service {
  font-family: 'Newsreader', serif;
  font-style: italic;
  font-size: 18px;
  color: var(--ink);
}
.mbt-rhode-price-desc {
  font-family: 'Inter', sans-serif;
  font-size: 12px;
  color: var(--ink-soft);
  margin-top: 3px;
}
.mbt-rhode-price-amount {
  font-family: 'Inter', sans-serif;
  font-size: 15px;
  font-weight: 500;
  color: var(--rose);
...
.mbt-rhode-cta-band-btn { color: var(--ink); }
.mbt-rhode-cta-band-btn:hover { background: var(--ink); color: var(--paper) !important; }

.mbt-rhode-footer { text-align: center; padding: 26px; }
.mbt-rhode-footer a {
  font-family: 'Inter', sans-serif;
  font-size: 11px;
  letter-spacing: 0.1em;
...
.mbt-rhode-footer a:hover { color: var(--rose); }
.mbt-rhode-footer-social { display: flex; justify-content: center; gap: 16px; margin-bottom: 14px; }
.mbt-rhode-footer-social a { color: var(--ink-soft); display: flex; }
.mbt-rhode-footer-social a:hover { color: var(--rose); }

@media (max-width: 720px) {
  .mbt-rhode-about { grid-template-columns: 1fr; padding: 60px 20px; }
  .mbt-rhode-portfolio-grid { grid-template-columns: repeat(2, 1fr); }
  .mbt-rhode-nav { gap: 18px; }
  .mbt-rhode-wordmark { order: -1; width: 100%; text-align: center; margin: 0 0 8px; }
}
`;
  ...
  return (
    <div className="mbt-app">
      ...
      <div className="mbt-auth-wrap">
        <a href="/" className="mbt-backhome" style={{ marginBottom: 14 }}><Home size={12} /> Back to Website</a>
    ...
    <div className="mbt-app">
      ...
      <div className="mbt-wrap">
        <div className="mbt-header-row">
          ...
          <div className="mbt-header-actions">
            <a href="/" className="mbt-backhome"><Home size={12} /> Website</a>
...
// ---------- Admin (full) view ----------
const DEFAULT_CONTRACT_TEMPLATE = `Makeup Artist Service Agreement

This Makeup Artist Service Agreement ("Agreement") is entered into between {{CLIENT_NAME}} ("Client") and Muznah ("Artist"), collectively referred to as "the Parties." The Artist agrees to provide professional makeup services under the terms outlined below.

1. Scope of Services
Artist will provide makeup services for the Client on the scheduled date of {{DATE}} at the location {{LOCATION}}. Services include but are not limited to consultations, trials, and event-day makeup applications for the number of people listed below. Any additional services must be agreed upon in writing.

Service booked: {{SERVICE}}

2. Payment and Deposit
A non-refundable deposit equal to 50% of the total service fee is required at the time of booking to secure the appointment. The remaining balance is due on or before the event date. Accepted forms of payment include cash, Zelle, or electronic transfer. No services will be rendered without full payment.

3. Cancellation and Rescheduling
All deposits are non-refundable unless the Artist cancels the appointment. If the Client cancels or reschedules for any reason, the deposit will not be refunded or transferred. If Artist must cancel, the deposit will be refunded in full within seven (7) business days. Rescheduling by the Client is subject to Artist's availability and may require a new deposit.

4. Travel and Expenses
Client agrees to reimburse the Artist for all travel expenses associated with transportation to and from the appointment location. Since the Artist travels exclusively by ride-share (e.g., Uber or Lyft), the full round-trip fare will be added to the final invoice. Travel fees vary depending on distance and time of booking. If the appointment requires multiple locations or extended travel time, additional charges may apply.

5. Client Responsibilities
Client shall provide a suitable environment with good lighting, electrical outlets, and a table or surface for Artist's materials. All participants must be present and ready at the scheduled time. Client must disclose any allergies or skin sensitivities before application.

6. Additional Services
Any additional services that were not added at the time of booking but were availed during the appointment will be charged separately and must be paid in full on the same day of service.

7. Liability
Artist uses professional-grade and sanitized products but shall not be held liable for allergic reactions or any injuries resulting from services provided. Client acknowledges that makeup application carries inherent risks and releases Artist from any related claims or liabilities.

8. Social Media & Photo Release
Client grants Artist permission to photograph, record, and post images or videos of the completed makeup look on social media platforms and in promotional materials. If the Client does not consent, they must notify the Artist in writing or verbally prior to the appointment.

9. Governing Law
This Agreement shall be governed by the laws of the State of Texas. Any disputes shall be resolved in the appropriate court located in Harris County, Texas.

10. Entire Agreement
This document constitutes the entire agreement between the Parties. No other promises or agreements shall be binding unless in writing and signed by both Parties.

11. Signature
To sign this agreement, please reply to this email confirming your agreement to the terms above, and include your typed full name and today's date. Send your reply to muznahf40@gmail.com.

Client Name: {{CLIENT_NAME}}
Date: _______________`;

const emptyBookingForm = { name: '', email: '', phone: '', date: '', time: '', service: '', location: '', notes: '' };
...
function AdminView({ onSignOut }) {
  ...
  async function loadWebsiteContent() {
    const [{ data: aboutData }, { data: heroData }, { data: aboutImgData }, { data: pData }, { data: igData }, { data: ttData }, { data: contractData }] = await Promise.all([
      supabase.from('site_content').select('*').eq('id', 'about').maybeSingle(),
      supabase.from('site_content').select('*').eq('id', 'hero_image').maybeSingle(),
      supabase.from('site_content').select('*').eq('id', 'about_image').maybeSingle(),
      supabase.from('portfolio_images').select('*').order('sort_order', { ascending: true }),
      supabase.from('site_content').select('*').eq('id', 'instagram_url').maybeSingle(),
      supabase.from('site_content').select('*').eq('id', 'tiktok_url').maybeSingle(),
      supabase.from('site_content').select('*').eq('id', 'contract_template').maybeSingle(),
    ]);
    setAboutDraft(aboutData ? aboutData.content : '');
    setHeroImageUrl(heroData ? heroData.content : '');
    setAboutImageUrl(aboutImgData ? aboutImgData.content : '');
    setPortfolio(pData || []);
    setInstagramDraft(igData && igData.content ? igData.content : 'https://instagram.com/makeupbymuznah');
    setTiktokDraft(ttData && ttData.content ? ttData.content : 'https://tiktok.com/@makeupbymuznah');
    setContractDraft(contractData && contractData.content ? contractData.content : DEFAULT_CONTRACT_TEMPLATE);
  }
  ...
  async function saveSocial() {
    await Promise.all([
      supabase.from('site_content').upsert({ id: 'instagram_url', content: instagramDraft, updated_at: new Date().toISOString() }),
      supabase.from('site_content').upsert({ id: 'tiktok_url', content: tiktokDraft, updated_at: new Date().toISOString() }),
    ]);
    setSocialSaved(true);
  ...
  async function saveContractTemplate() {
    await supabase.from('site_content').upsert({ id: 'contract_template', content: contractDraft, updated_at: new Date().toISOString() });
    setContractSaved(true);
    setTimeout(() => setContractSaved(false), 1800);
  }

  function buildFilledContract(b) {
    return contractDraft
      .split('{{CLIENT_NAME}}').join(b.name || '')
      .split('{{DATE}}').join(b.date ? formatDatePretty(b.date) : '')
      .split('{{LOCATION}}').join(b.location || '')
      .split('{{SERVICE}}').join(b.service || '');
  }

  function downloadContractPdf(b, filledText) {
    ...
    paragraphs.forEach((para) => {
      ...
      const isHeading = /^\d+\.\s/.test(para.trim()) || para.trim() === 'Makeup Artist Service Agreement';
    ...
    const safeName = (b.name || 'client').replace(/[^a-z0-9]+/gi, '-');
    doc.save(`Service-Agreement-${safeName}.pdf`);
  }

  function sendContractForBooking(b) {
    if (!b.email) {
      alert("This booking doesn't have a client email saved, so there's nowhere to send the contract. Add one by editing the booking first.");
      return;
    }
    const filled = buildFilledContract(b);
    downloadContractPdf(b, filled);

    const subject = `Your Makeup Service Agreement — ${b.name}`;
    const shortBody = `Hi ${b.name || 'there'},\n\nAttached is your service agreement for ${b.date ? formatDatePretty(b.date) : 'your appointment'}${b.location ? ` at ${b.location}` : ''}.\n\nPlease review it, then reply to this email with your typed full name and today's date to confirm you agree to the terms, and send it to muznahf40@gmail.com.\n\nThank you!`;

    // The PDF just downloaded to your computer — attach it below using the paperclip
    // button in your email app before sending (browsers can't auto-attach files for security reasons).
    setTimeout(() => {
      window.location.href = `mailto:${b.email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(shortBody)}`;
    }, 300);
  }

  async function uploadSingleSiteImage(file, contentId, setUrlFn, setBusyFn) {
    setBusyFn(true);
    try {
      const ext = file.name.split('.').pop();
      const path = `${contentId}-${Date.now()}.${ext}`;
      ...
      await supabase.from('site_content').upsert({ id: contentId, content: data.publicUrl, updated_at: new Date().toISOString() });
      setUrlFn(data.publicUrl);
    } catch (err) {
      console.error(`${contentId} upload failed`, err);
    }
    setBusyFn(false);
  }
  ...
  function handleHeroUpload(e) {
    ...
    if (file) uploadSingleSiteImage(file, 'hero_image', setHeroImageUrl, setUploadingHero);
    e.target.value = '';
  }

  function handleAboutPhotoUpload(e) {
    const file = e.target.files[0];
    if (file) uploadSingleSiteImage(file, 'about_image', setAboutImageUrl, setUploadingAboutPhoto);
    e.target.value = '';
  }
  ...
  async function handlePortfolioUpload(e) {
    ...
    let nextOrder = portfolio.length;
    for (const file of files) {
      try {
        ...
        const { data: row, error: insErr } = await supabase
          ...
          .insert({ url: data.publicUrl, sort_order: nextOrder })
          .select()
          .single();
        if (!insErr) {
          setPortfolio((prev) => [...prev, row]);
          nextOrder += 1;
        }
      } catch (err) {
  ...
  async function deletePortfolioImage(id) {
    ...
    if (orderDirty) setOrderDirty(true); // still-unsaved order remains unsaved after a delete
    await supabase.from('portfolio_images').delete().eq('id', id);
  }

  function handlePortfolioDragStart(idx) {
    setDragIndex(idx);
  }

  function handlePortfolioDragOver(e, idx) {
    e.preventDefault();
  ...
  function handlePortfolioDragEnd() {
    setDragIndex(null);
  }

  async function savePortfolioOrder() {
    setSavingOrder(true);
    await Promise.all(portfolio.map((p, idx) => supabase.from('portfolio_images').update({ sort_order: idx }).eq('id', p.id)));
  ...
  function useInquiryForBooking(q) {
    ...
    setBookingForm({
      ...
      time: q.readyByTime,
      service: q.service,
      location: q.clientAddress || q.location || '',
      notes,
    });
  ...
  function rowToBooking(r) {
    return { id: r.id, name: r.name, email: r.email || '', phone: r.phone || '', date: r.date || '', time: r.time || '', service: r.service || '', location: r.location || '', notes: r.notes || '', done: r.done };
  }
  ...
  function openEditBookingForm(b) {
    setBookingForm({ name: b.name, email: b.email, phone: b.phone, date: b.date, time: b.time, service: b.service, location: b.location, notes: b.notes });
    ...
    <div className="mbt-app">
      ...
      <div className="mbt-wrap">
        ...
        <div className="mbt-tabs">
          ...
          <button className={`mbt-tab ${tab === 'contract' ? 'active' : ''}`} onClick={() => setTab('contract')}>Contract</button>
        </div>
        ...
        {loading ? (
          ...
          <div>
            ...
            {showBookingForm && (
              <div className="mbt-form">
                ...
                <div className="mbt-field-row">
                  <div className="mbt-field" style={{ width: '100%' }}>
                    <label>Location (for the contract)</label>
                    <input value={bookingForm.location} onChange={(e) => setBookingForm({ ...bookingForm, location: e.target.value })} placeholder="e.g. 123 Main St, Houston, TX" />
                  </div>
                </div>
                <div className="mbt-field-row">
                  <div className="mbt-field">
                    <label>Notes</label>
                    <textarea rows={2} value={bookingForm.notes} onChange={(e) => setBookingForm({ ...bookingForm, notes: e.target.value })} placeholder="content plan, anything to remember" />
                  </div>
                </div>
                ...
                <div key={b.id} className={`mbt-card ${b.done ? 'done' : ''}`}>
                  ...
                  <div className="mbt-card-body">
                    ...
                    <div className="mbt-actions">
                      ...
                      <button className="mbt-iconbtn" onClick={() => sendContractForBooking(b)}>
                        <FileDown size={12} /> Send Contract
                      </button>
          ...
          <div>
            <div className="mbt-form-title" style={{ marginBottom: 10 }}>Hero Photo (the big photo at the top of your homepage)</div>
            <div className="mbt-form">
              <input type="file" accept="image/*" onChange={handleHeroUpload} disabled={uploadingHero} />
              {uploadingHero && <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'var(--ink-soft)', marginTop: 8 }}>Uploading…</div>}
              {heroImageUrl && (
                <div style={{ marginTop: 10 }}>
                  <img src={heroImageUrl} alt="hero preview" style={{ width: 160, borderRadius: 4, border: '1px solid var(--line)', display: 'block' }} />
                </div>
              )}
            </div>

            <div className="mbt-form-title" style={{ margin: '24px 0 10px' }}>About Photo (shown next to your bio)</div>
            <div className="mbt-form">
              <input type="file" accept="image/*" onChange={handleAboutPhotoUpload} disabled={uploadingAboutPhoto} />
              {uploadingAboutPhoto && <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: 'var(--ink-soft)', marginTop: 8 }}>Uploading…</div>}
              {aboutImageUrl && (
                <div style={{ marginTop: 10 }}>
                  <img src={aboutImageUrl} alt="about preview" style={{ width: 160, borderRadius: 4, border: '1px solid var(--line)', display: 'block' }} />
                </div>
              )}
            </div>

            <div className="mbt-form-title" style={{ margin: '24px 0 10px' }}>Social Links (icons shown on your homepage)</div>
            <div className="mbt-form">
              <div className="mbt-field-row">
                <div className="mbt-field">
                  <label>Instagram URL</label>
                  <input value={instagramDraft} onChange={(e) => setInstagramDraft(e.target.value)} placeholder="https://instagram.com/yourhandle" />
                </div>
                <div className="mbt-field">
                  <label>TikTok URL</label>
                  <input value={tiktokDraft} onChange={(e) => setTiktokDraft(e.target.value)} placeholder="https://tiktok.com/@yourhandle" />
                </div>
              </div>
              <div className="mbt-form-actions" style={{ marginTop: 10 }}>
                <button className="mbt-btn-primary" onClick={saveSocial}>{socialSaved ? 'Saved!' : 'Save Social Links'}</button>
              </div>
            </div>

            <div className="mbt-form-title" style={{ margin: '24px 0 10px' }}>About Me (shown on your public homepage)</div>
            <div className="mbt-form">
              <textarea
                rows={6}
            ...
            {portfolio.length === 0 ? (
              ...
              <>
                <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--ink-soft)', margin: '14px 0 8px' }}>
                ...
                <div className="mbt-portfolio-grid">
                  {portfolio.map((p, idx) => (
                    <div
                      key={p.id}
                      className={`mbt-portfolio-item mbt-portfolio-draggable ${dragIndex === idx ? 'mbt-portfolio-dragging' : ''}`}
                      style={{ position: 'relative' }}
                      draggable
                      onDragStart={() => handlePortfolioDragStart(idx)}
                      onDragOver={(e) => handlePortfolioDragOver(e, idx)}
                      onDragEnd={handlePortfolioDragEnd}
                    >
                      <img src={p.url} alt={p.caption || 'portfolio'} draggable={false} />
                      <button className="mbt-inspiration-remove" onClick={() => deletePortfolioImage(p.id)}><Trash2 size={11} /></button>
                    </div>
                  ))}
                </div>
                <div className="mbt-form-actions" style={{ marginTop: 12 }}>
                  <button className="mbt-btn-primary" onClick={savePortfolioOrder} disabled={!orderDirty || savingOrder}>
          ...
          <div>
            <div className="mbt-form-title" style={{ marginBottom: 10 }}>Contract Template</div>
            <div style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: 'var(--ink-soft)', marginBottom: 12, lineHeight: 1.5 }}>
              This is what gets turned into a PDF and downloaded when you click "Send Contract" on a booking — it also opens a short email to the client, ready for you to attach that PDF and send. Use{' '}
              <code style={{ background: 'var(--bone)', padding: '1px 4px', borderRadius: 2 }}>{'{{CLIENT_NAME}}'}</code>,{' '}
              <code style={{ background: 'var(--bone)', padding: '1px 4px', borderRadius: 2 }}>{'{{DATE}}'}</code>,{' '}
              <code style={{ background: 'var(--bone)', padding: '1px 4px', borderRadius: 2 }}>{'{{LOCATION}}'}</code>, and{' '}
              <code style={{ background: 'var(--bone)', padding: '1px 4px', borderRadius: 2 }}>{'{{SERVICE}}'}</code>{' '}
              anywhere you want those auto-filled from the booking.
            </div>
            <div className="mbt-form">
              <textarea
                rows={22}
                style={{ width: '100%', fontFamily: 'JetBrains Mono, monospace', fontSize: 12.5, lineHeight: 1.6, padding: 12, border: '1px solid var(--line)', borderRadius: 3, background: 'var(--bone)', color: 'var(--ink)' }}
                value={contractDraft}
                onChange={(e) => setContractDraft(e.target.value)}
              />
              <div className="mbt-form-actions" style={{ marginTop: 10 }}>
                <button className="mbt-btn-primary" onClick={saveContractTemplate}>{contractSaved ? 'Saved!' : 'Save Contract Template'}</button>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mbt-app">
        ...
        <div className="mbt-auth-wrap">
          ...
          <div className="mbt-auth-card" style={{ textAlign: 'center' }}>
            ...
            <a href="/" className="mbt-backhome" style={{ display: 'inline-flex', marginTop: 16 }}><Home size={12} /> Back to Website</a>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="mbt-app">
      ...
      <div className="mbt-inquiry-wrap">
        <a href="/" className="mbt-backhome" style={{ marginBottom: 14, display: 'inline-flex' }}><Home size={12} /> Back to Website</a>
        <div className="mbt-eyebrow" style={{ textAlign: 'center' }}>Booking Portal</div>
...
function HomePage() {
  ...
  const [portfolio, setPortfolio] = useState([]);
  const [prices, setPrices] = useState([]);
  const [instagramUrl, setInstagramUrl] = useState('https://instagram.com/makeupbymuznah');
  const [tiktokUrl, setTiktokUrl] = useState('https://tiktok.com/@makeupbymuznah');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      const [{ data: aboutData }, { data: heroData }, { data: aboutImgData }, { data: pfData }, { data: priceData }, { data: igData }, { data: ttData }] = await Promise.all([
        supabase.from('site_content').select('*').eq('id', 'about').maybeSingle(),
        supabase.from('site_content').select('*').eq('id', 'hero_image').maybeSingle(),
        supabase.from('site_content').select('*').eq('id', 'about_image').maybeSingle(),
        supabase.from('portfolio_images').select('*').order('sort_order', { ascending: true }),
      ...
      setHeroImage(heroData ? heroData.content : '');
      setAboutImage(aboutImgData ? aboutImgData.content : '');
      setPortfolio(pfData || []);
      setPrices(priceData || []);
      if (igData && igData.content) setInstagramUrl(igData.content);
      if (ttData && ttData.content) setTiktokUrl(ttData.content);
      setLoading(false);
    })();
  }, []);

  return (
    <div className="mbt-rhode">
      <style>{STYLE}</style>

      <nav className="mbt-rhode-nav">
        <a href="#about" className="mbt-rhode-navlink">About</a>
        <a href="#portfolio" className="mbt-rhode-navlink">Portfolio</a>
        <div className="mbt-rhode-wordmark">Muses by Muz</div>
        <a href="#pricing" className="mbt-rhode-navlink">Pricing</a>
        <a href="/?inquire=1" className="mbt-rhode-navcta">Book Now</a>
        <div className="mbt-rhode-social">
          <a href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram"><Instagram size={17} /></a>
          <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" aria-label="TikTok">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M16.6 5.82c-1.03-.9-1.6-2.2-1.6-3.62h-3.15v13.5c0 1.53-1.25 2.78-2.78 2.78a2.78 2.78 0 0 1 0-5.56c.28 0 .55.04.8.12V9.9a5.94 5.94 0 0 0-.8-.05C5.6 9.85 3 12.45 3 15.62s2.6 5.78 5.78 5.78 5.78-2.6 5.78-5.78V9.4a7.5 7.5 0 0 0 4.4 1.42V7.68a4.5 4.5 0 0 1-2.36-1.86z"/></svg>
          </a>
        </div>
      </nav>

      <header className={`mbt-rhode-hero ${!heroImage ? 'mbt-rhode-hero-empty' : ''}`}>
        {heroImage && (
          <>
            <img src={heroImage} alt="Muznah" className="mbt-rhode-hero-img" />
            <div className="mbt-rhode-hero-scrim" />
          </>
        )}
        <div className="mbt-rhode-hero-copy">
          <div className="mbt-rhode-eyebrow">Houston, TX</div>
          <h1 className="mbt-rhode-hero-title">Soft glam,<br />done right.</h1>
          <a href="/?inquire=1" className="mbt-rhode-hero-cta">Book Now</a>
        </div>
      </header>

      {!loading && (about || aboutImage) && (
        <section id="about" className="mbt-rhode-about">
          {aboutImage && (
            <div className="mbt-rhode-about-photo">
              <img src={aboutImage} alt="Muznah" />
            </div>
          )}
          <div className="mbt-rhode-about-copy">
            <div className="mbt-rhode-label">about</div>
            {about && about.split('\n').filter(Boolean).map((para, i) => (
              <p key={i} className={i === 0 ? 'mbt-rhode-about-lead' : ''}>{para}</p>
            ))}
          </div>
        </section>
      )}
      ...
      {!loading && portfolio.length > 0 && (
        <section id="portfolio" className="mbt-rhode-portfolio">
          <div className="mbt-rhode-label" style={{ textAlign: 'center' }}>portfolio</div>
          <div className="mbt-rhode-portfolio-grid">
            {portfolio.map((p) => (
              <a key={p.id} href={p.url} target="_blank" rel="noopener noreferrer" className="mbt-rhode-portfolio-item">
      ...
      {!loading && prices.length > 0 && (
        <section id="pricing" className="mbt-rhode-pricing">
          <div className="mbt-rhode-label" style={{ textAlign: 'center' }}>pricing</div>
          <div className="mbt-rhode-price-list">
            {prices.map((p) => (
              <div key={p.id} className="mbt-rhode-price-row">
                <div>
                  <div className="mbt-rhode-price-service">{p.service}</div>
                  {p.description && <div className="mbt-rhode-price-desc">{p.description}</div>}
                </div>
                {p.price != null && <div className="mbt-rhode-price-amount">${Number(p.price).toFixed(0)}</div>}
              </div>
            ))}
          </div>
        </section>
      ...
      <section className="mbt-rhode-cta-band">
        <div className="mbt-rhode-cta-text">Let's create something beautiful.</div>
        <a href="/?inquire=1" className="mbt-rhode-hero-cta mbt-rhode-cta-band-btn">Book Now</a>
      </section>

      <footer className="mbt-rhode-footer">
        <div className="mbt-rhode-footer-social">
          <a href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram"><Instagram size={16} /></a>
          <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" aria-label="TikTok">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M16.6 5.82c-1.03-.9-1.6-2.2-1.6-3.62h-3.15v13.5c0 1.53-1.25 2.78-2.78 2.78a2.78 2.78 0 0 1 0-5.56c.28 0 .55.04.8.12V9.9a5.94 5.94 0 0 0-.8-.05C5.6 9.85 3 12.45 3 15.62s2.6 5.78 5.78 5.78 5.78-2.6 5.78-5.78V9.4a7.5 7.5 0 0 0 4.4 1.42V7.68a4.5 4.5 0 0 1-2.36-1.86z"/></svg>
          </a>
        </div>
